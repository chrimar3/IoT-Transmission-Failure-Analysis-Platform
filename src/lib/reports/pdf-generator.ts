...
function addReportHeader(doc: PDFKit.PDFDocument, template: ReportTemplate, data: PDFReportData) {
  doc.fontSize(16)
     .fillColor('#1e293b')
  ...
  if (data.dataRange) {
    doc.text(`Data Period: ${data.dataRange.start} to ${data.dataRange.end}`, 50, 140)
  }
...
function addComponent(doc: PDFKit.PDFDocument, component: ReportComponent, data: PDFReportData) {
  ...
  if (component.styling?.background_color) {
    doc.rect(x, y, width, _height)
       .fill(component.styling.background_color)
  }
...
function addTextComponent(
  ...
  doc.fontSize(component.styling?.font_size || 12)
...
function addHeaderComponent(
  ...
  doc.fontSize(component.styling?.font_size || 18)
     .fillColor(component.styling?.text_color || '#1e293b')
...
function addMetricComponent(
  ...
  doc.fontSize(12)
     .fillColor('#64748b')
     .text(component.config.label || 'Metric', x + 10, y + height - 30, {
       width: width - 20,
       align: 'center'
     })
}
...
async function addChartComponent(
  ...
  try {
    ...
    const chartBuffer = await generateChartImage(component, data, width, _height)
    ...
    doc.rect(x, y, width, _height)
       ...
       .fontSize(12)
       .fillColor('#64748b')
       .text('Chart placeholder', x + 10, y + height / 2, {
         width: width - 20,
         align: 'center'
       })
  }
...
function addTableComponent(
  ...
  tableData.columns.forEach((column: unknown, index: number) => {
    ...
    doc.rect(colX, y, columnWidth, rowHeight)
       .fill('#f3f4f6')
       .stroke('#e5e7eb')
  ...
  visibleRows.forEach((row: unknown, rowIndex: number) => {
    ...
    tableData.columns.forEach((column: unknown, colIndex: number) => {
      ...
      doc.rect(colX, rowY, columnWidth, rowHeight)
         .stroke('#e5e7eb')
         .fillColor('#000000')
...
function addImagePlaceholder(
  ...
  doc.rect(x, y, width, _height)
     ...
     .fontSize(12)
     .fillColor('#64748b')
     .text('Image placeholder', x + 10, y + height / 2, {
       width: width - 20,
       align: 'center'
     })
}
...
function addReportFooter(doc: PDFKit.PDFDocument, branding: PDFBranding) {
  ...
  doc.fontSize(8)
     .fillColor('#64748b')
     .text(
       branding.footer_text || 'Generated by CU-BEMS IoT Transmission Failure Analysis Platform',
       50,
       footerY,
       { align: 'center' }
     )
...
async function generateChartImage(
  ...
  return Buffer.from(`PDF chart placeholder for ${component.config.chart_type || 'chart'} - canvas module not available`)
}

function getMetricValue(component: ReportComponent, data: PDFReportData): string | null {
