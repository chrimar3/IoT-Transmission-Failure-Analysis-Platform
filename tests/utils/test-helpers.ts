...
export async function mockApiRequest(
  ...
  if (
    !apiKey ||
    invalidKeyFormats.includes(apiKey) ||
  ...
  if (
    apiKey.includes('free') ||
    (globalThis as any).__testFreeApiKey === apiKey
  ) {
  ...
  if (
    ...
    endpoint.includes('malformed') ||
    params.start_date === 'invalid-date' ||
    params.end_date === 'invalid-date' ||
    params.sensor_ids === 'INVALID_SENSOR_FORMAT' ||
    params.max_points === '-1' ||
    params.sensor_ids === null ||
    params.sensor_ids === 'null' ||
    (params.sensor_ids &&
      typeof params.sensor_ids === 'string' &&
      params.sensor_ids.includes('@')) ||
    (params.start_date && params.start_date.includes('13-45')) ||
    (params.start_date && params.start_date.includes('25:70:80')) ||
    (body &&
      typeof body === 'object' &&
      (body as any).format === 'invalid_format')
  ) {
    return {
      status: 400,
      headers: { 'content-type': 'application/json' },
  ...
  const maxPoints =
    typeof params.max_points === 'number'
      ? params.max_points
      : parseInt(params.max_points);
  ...
  if (
    params.interval &&
    !['minute', 'hour', 'day', 'week'].includes(params.interval)
  ) {
    return {
      status: 400,
      headers: { 'content-type': 'application/json' },
  ...
  if (
    (endpoint.includes('/exports/') && endpoint.includes('/status')) ||
...
function generateMockApiData(
  ...
  if (endpoint.includes('/timeseries')) {
    ...
    const equipmentTypes = params.equipment_types
      ? params.equipment_types.split(',')
      : [];
    const floorNumbers = params.floor_numbers
    ...
    const pointCount = Math.min(
      24,
      Math.max(1, Math.floor(timeRange / (60 * 60 * 1000)))
    ); // hourly points

    return {
      series: sensorIds
        .map((sensorId: string, idx: number) => {
          // Respect equipment type filtering
          let equipmentType: string;
          if (equipmentTypes.length > 0) {
            equipmentType = equipmentTypes[idx % equipmentTypes.length];
          } else {
            equipmentType = getRandomArrayElement([
              'HVAC',
              'Lighting',
              'Power',
              'Water',
          ...
          return {
            ...
            data: Array.from({ length: pointCount }, (_, i) => {
              const timestamp = new Date(
                startDate.getTime() + (i * timeRange) / pointCount
              );
              return {
  ...
  if (endpoint.includes('/analytics')) {
    return {
      ...
      insights: {
        ...
        trend_analysis: {
          overall_trend: getRandomArrayElement([
            'increasing',
            'decreasing',
            'stable',
          ]),
        ...
        anomalies: Array.from({ length: 3 }, () => ({
          ...
          severity: getRandomArrayElement([
            'low',
            'medium',
            'high',
            'critical',
  ...
  if (endpoint.includes('/floors/')) {
    ...
    return {
      ...
      equipment: Array.from({ length: 8 }, (_, i) => ({
        ...
        equipment_type: getRandomArrayElement([
          'HVAC',
          'Lighting',
          'Power',
          'Water',
  ...
  if (endpoint.includes('/exports/create') && method === 'POST') {
    ...
    return {
      ...
      format:
        (body && typeof body === 'object' && (body as any).format) ||
        params.format ||
        'json',
      download_url: null,
    };
  ...
  if (endpoint.includes('/exports/') && endpoint.includes('/status')) {
    ...
    return {
      ...
      created_at: new Date(
        Date.now() - randomInt(60000, 3600000)
      ).toISOString(),
      estimated_completion: new Date(
        Date.now() + randomInt(60000, 300000)
      ).toISOString(),
      file_info: {
        format: 'json',
        compression: 'gzip',
  ...
  if (endpoint.includes('/equipment/')) {
    ...
    return {
      ...
      aggregate_performance: {
        ...
        performance_trend: getRandomArrayElement([
          'improving',
          'stable',
          'declining',
        ]),
  ...
  if (endpoint.includes('/patterns')) {
    return {
      ...
      anomalies: Array.from({ length: randomInt(2, 6) }, () => ({
        ...
        timestamp: new Date(
          Date.now() - randomInt(1, 168) * 60 * 60 * 1000
        ).toISOString(),
...
export async function makeApiRequest(
  ...
  const apiKey = options.apiKey || 'sk_test_default_key';
  const response = await mockApiRequest(endpoint, apiKey, {
...
export function createMockRequest(url: string, options: any = {}) {
  const headersObj = options._headers || {};
  return {
    url,
...
/**
 * Generate API key with sk_ prefix (for security testing)
 */
...
/**
 * Hash API key using SHA-256 (for security testing)
 */
...
/**
 * Validate API key format (for security testing)
 */
...
/**
 * Cleanup test data (mock implementation for testing)
 */
