...
import {
  createMockUser as createMockUserHelper,
  createMockApiKey as createMockApiKeyHelper,
  createMockRequest,
  generateApiKey,
  hashApiKey,
  validateApiKey,
...
describe('API Security Validation - Story 4.2', () => {
  ...
  beforeAll(async () => {
    ...
    professionalApiKey = await createMockApiKey(testUser.id, {
      tier: 'professional',
    });
  ...
  describe('AC6: API Security and Authentication', () => {
    ...
    describe('Authentication Security', () => {
      ...
      test('validates API key authentication', async () => {
        ...
        const invalidRequests = [
          ...
          createMockRequest({
            _headers: { Authorization: 'Bearer invalid_key' },
          }),
          createMockRequest({ _headers: { Authorization: 'Basic invalid' } }),
          createMockRequest({
            _headers: { Authorization: professionalApiKey },
          }), // Missing Bearer
        ];
    ...
    describe('Rate Limiting Security', () => {
      ...
      test('implements proper rate limit headers', async () => {
        ...
        expect(new Date(response.headers['x-ratelimit-reset'])).toBeInstanceOf(
          Date
        );
      });
    });
    ...
    describe('API Versioning Security', () => {
      ...
      test('maintains backward compatibility security', async () => {
        ...
        for (const endpoint of deprecatedEndpoints) {
          ...
          const authenticatedResponse = await makeAuthenticatedRequest(
            endpoint,
            professionalApiKey
          );
          expect(authenticatedResponse._headers).toHaveProperty(
...
async function createMockApiKey(
  _userId: string,
  _options: { tier?: string; scopes?: string[] } = {}
) {
  const apiKey = generateApiKey();
