...
// Mock PDFKit with complete chainable implementation
...
jest.mock('pdfkit', () => jest.fn(() => mockPDFDocumentInstance));
...
describe('PDF Generator', () => {
  ...
  const mockData = {
    ...
    tables: {},
    insights: ['Average temperature: 25.5°C', 'Data shows increasing trend'],
  };

  let mockDoc: typeof mockPDFDocumentInstance;

  beforeEach(() => {
    // Reset all mock function calls
    jest.clearAllMocks();

    // Reset the page height for tests that need it
    mockPDFDocumentInstance.page.height = 800;

    // Assign mockDoc to the global mockPDFDocumentInstance for test compatibility
    mockDoc = mockPDFDocumentInstance;
  });

  describe('generatePDF', () => {
    it('should create a PDF document with correct settings', async () => {
      // Mock the PDF generation to resolve immediately
      (mockDoc.on as jest.Mock).mockImplementation(
      ...
      const endCallback = (mockDoc.on as jest.Mock).mock.calls.find(
        (call) => call[0] === 'end'
      )?.[1];
      if (endCallback) {
    ...
    it('should handle PDF generation errors', async () => {
      // Make the end method throw an error to simulate PDF generation failure
      mockDoc.end.mockImplementation(() => {
        throw new Error('PDF generation failed');
      });

      await expect(generatePDF(mockTemplate, mockData, {})).rejects.toThrow(
        'PDF generation failed'
      );

      // Restore the original mock implementation
      mockDoc.end.mockImplementation(jest.fn());
    });
  ...
  describe('Component Rendering', () => {
    it('should render all component types without errors', async () => {
        ...
        template_data: {
          ...
          components: [
            ...
            },
            {
              id: '3',
              type: 'chart',
              position: { x: 0, y: 120, width: 100, height: 50 },
              config: { chart_type: 'bar' },
            },
            {
              id: '4',
              type: 'table',
              position: { x: 0, y: 180, width: 100, height: 50 },
              config: {},
            },
            {
              id: '5',
              type: 'metric',
              position: { x: 0, y: 240, width: 100, height: 50 },
              config: {},
            },
            {
              id: '6',
            ...
            },
            {
              id: '7',
              type: 'divider',
              position: { x: 0, y: 360, width: 100, height: 50 },
              config: {},
            },
          ],
        },
      ...
      await expect(
        generatePDF(allComponentsTemplate, mockData, {})
