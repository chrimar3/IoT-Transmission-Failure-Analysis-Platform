  ...
  describe('POST - Create Export Job', () => {
    ...
    describe('Authentication', () => {
      it('should accept Professional tier web session', async () => {
        ...
        const request = new NextRequest('http://localhost:3000/api/export/create', {
          method: 'POST',
    ...
    describe('Export Job Creation', () => {
      ...
      it('should handle export job creation errors', async () => {
        const mockExportManager = {
          createExportJob: jest.fn().mockRejectedValue(new Error('Export creation failed'))
        }
        ;(ExportManager.getInstance as jest.Mock).mockReturnValue(mockExportManager)
  ...
  describe('GET - Export History', () => {
    ...
    describe('Authentication', () => {
      it('should return history for Professional tier web session', async () => {
        ...
        expect(mockExportManager.getUserExportHistory).toHaveBeenCalledWith('user-123')
      })
      ...
      it('should return history for Professional tier API key', async () => {
        ...
        expect(response.status).toBe(200)
        expect(result.success).toBe(true)
        expect(mockExportManager.getUserExportHistory).toHaveBeenCalledWith('user-456')
      })
    })
