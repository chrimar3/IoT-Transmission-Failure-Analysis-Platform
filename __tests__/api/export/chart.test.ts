...
describe('/api/export/chart', () => {
  ...
  describe('Authentication', () => {
    test('requires authentication', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/export/chart', {
        method: 'POST',
  ...
  describe('Subscription Validation', () => {
    test('rejects free tier users', async () => {
      ...
      expect(data.message).toBe('Chart export requires Professional or Enterprise subscription')
    })
  ...
  describe('Request Validation', () => {
    ...
    test('validates dimension constraints', async () => {
      ...
      expect(data.validation_errors).toContainEqual(
        expect.objectContaining({
          ...
          message: expect.stringContaining('Number must be less than or equal to 2000')
        })
      )
    })
    ...
    test('validates title length', async () => {
      ...
      expect(data.validation_errors).toContainEqual(
        expect.objectContaining({
          field: 'title',
          message: expect.stringContaining('String must contain at most 100 character(s)')
        })
      )
    })
  ...
  describe('Export Generation', () => {
    beforeEach(() => {
      mockGetServerSession.mockResolvedValue(mockProfessionalSession)
      // Mock setTimeout for delayed responses
      jest.useFakeTimers()
    })
    ...
    test('generates PNG export successfully', async () => {
      ...
      // Fast-forward through any delays in mock export generation
      jest.runAllTimers()
      ...
      expect(data.data.filename).toMatch(/bangkok-timeseries_\d+sensors_.*\.png/)
    })
    ...
    test('generates different file sizes for different formats', async () => {
      // PNG request
      const pngRequest = new NextRequest('http://localhost:3000/api/export/chart', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(validExportRequest)
      })

      const pngPromise = POST(pngRequest)
      jest.runAllTimers()
      const pngResponse = await pngPromise
      ...
      const pdfRequestObj = new NextRequest('http://localhost:3000/api/export/chart', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(pdfRequest)
      })

      const pdfPromise = POST(pdfRequestObj)
      jest.runAllTimers()
    ...
    test('sets appropriate expiration time', async () => {
      ...
      expect(expiresAt).toBeLessThan(afterRequest + (24 * 60 * 60 * 1000) + tolerance)
    })
  })
