  ...
  key_insights: [
    {
      ...
      actionable_recommendation: 'Adjust HVAC scheduling during low occupancy periods',
...
describe('Dashboard Integration Tests - Critical User Workflows', () => {
  ...
  describe('Workflow 1: Executive Dashboard Load and Navigation', () => {
    it('should load dashboard and display key metrics within performance budget', async () => {
      ...
      expect(screen.getByText('Loading Bangkok dataset insights...')).toBeInTheDocument()

      // Wait for data to load
      await waitFor(() => {
        expect(screen.getByText('CU-BEMS Executive Dashboard')).toBeInTheDocument()
        expect(screen.getByText('Bangkok Building IoT Sensor Analysis')).toBeInTheDocument()
      }, { timeout: 5000 })
