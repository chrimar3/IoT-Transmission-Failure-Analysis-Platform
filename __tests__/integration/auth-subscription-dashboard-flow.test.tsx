...
import { mockSession, mockStripeWebhook } from '@/__tests__/utils/auth-test-helpers'
import Dashboard from '@/app/dashboard/page'
import { createSubscription, updateSubscriptionStatus } from '@/lib/api/subscription-management'
...
describe('CRITICAL: Auth→Subscription→Dashboard Integration Flow', () => {
  ...
  const testScenarios: IntegrationTestScenario[] = [
    {
      ...
      expectedRestrictions: ['export-disabled', 'api-access-denied', 'advanced-analytics-hidden']
    },
    {
      name: 'Free tier user - basic dashboard with upgrade prompts',
      userState: 'free',
      expectedDashboardFeatures: ['basic-metrics', 'time-series-basic', 'upgrade-prompts'],
      expectedRestrictions: ['export-disabled', 'api-access-denied', 'advanced-analytics-hidden']
    },
    {
      name: 'Professional user - full dashboard access',
      userState: 'professional',
      expectedDashboardFeatures: ['basic-metrics', 'time-series-advanced', 'export-enabled', 'api-access', 'advanced-analytics'],
  ...
  testScenarios.forEach(scenario => {
    test(`INTEGRATION FLOW: ${scenario.name}`, async () => {
      const session = createMockSession(scenario.userState)
      ...
      await waitFor(() => {
        expect(container.querySelector('[data-testid="dashboard-container"]')).toBeInTheDocument()
      })

      // Verify expected features are present
      for (const feature of scenario.expectedDashboardFeatures) {
        expect(container.querySelector(`[data-testid="${feature}"]`)).toBeInTheDocument()
      }

      // Verify restrictions are enforced
      for (const restriction of scenario.expectedRestrictions) {
        expect(container.querySelector(`[data-testid="${restriction}"]`)).toBeInTheDocument()
      }
  ...
  test('REVENUE CRITICAL: Payment success → immediate feature access', async () => {
    ...
    const freeSession = createMockSession('free')
    ...
    expect(container.querySelector('[data-testid="export-disabled"]')).toBeInTheDocument()
    expect(container.querySelector('[data-testid="api-access-denied"]')).toBeInTheDocument()
    ...
    const professionalSession = createMockSession('professional')
    rerender(
    ...
    await waitFor(() => {
      expect(container.querySelector('[data-testid="export-enabled"]')).toBeInTheDocument()
      expect(container.querySelector('[data-testid="api-access"]')).toBeInTheDocument()
      expect(container.querySelector('[data-testid="advanced-analytics"]')).toBeInTheDocument()
    })
  ...
  test('PAYMENT FAILURE: Graceful handling of failed payments', async () => {
    ...
    expect(container.querySelector('[data-testid="export-enabled"]')).toBeInTheDocument()
    expect(container.querySelector('[data-testid="payment-warning"]')).toBeInTheDocument()
    ...
    await waitFor(() => {
      expect(container.querySelector('[data-testid="export-disabled"]')).toBeInTheDocument()
      expect(container.querySelector('[data-testid="reactivation-prompt"]')).toBeInTheDocument()
    })
  ...
  test('SECURITY CRITICAL: Rate limiting enforcement by subscription tier', async () => {
    const scenarios = [
      ...
      { tier: 'professional', limit: 10000, endpoint: '/api/v1/data/timeseries' }
    ]

    for (const scenario of scenarios) {
      const session = createMockSession(scenario.tier as any)
      ...
      console.log(`✅ ${scenario.tier} tier rate limiting: ${successfulRequests}/${scenario.limit} allowed`)
    }
  })
  ...
  test('SESSION PERSISTENCE: Dashboard state across page reloads', async () => {
    ...
    await waitFor(() => {
      expect(container.querySelector('[data-selected="last-30-days"]')).toBeInTheDocument()
      expect(container.querySelector('[data-selected="floor-3"]')).toBeInTheDocument()
    })
    ...
    await waitFor(() => {
      expect(reloadedContainer.querySelector('[data-selected="last-30-days"]')).toBeInTheDocument()
      expect(reloadedContainer.querySelector('[data-selected="floor-3"]')).toBeInTheDocument()
    })
...
function createMockSession(userState: 'anonymous' | 'free' | 'professional' | 'expired') {
...
function mockApiResponses(url: string, options?: RequestInit): Promise<Response> {
