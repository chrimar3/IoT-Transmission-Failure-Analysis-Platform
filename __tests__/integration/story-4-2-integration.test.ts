...
describe('Story 4.2: Professional API Integration Tests', () => {
  ...
  describe('Complete Professional User Journey', () => {
    test('end-to-end professional API workflow', async () => {
      ...
      expect(testWebhooks.some(w => w.event_type === 'webhook.test')).toBe(true)
    ...
    test('handles rate limiting gracefully during high usage', async () => {
      ...
      expect(parseInt(lastSuccessful.headers['x-ratelimit-remaining'])).toBeGreaterThan(9800)
    ...
    test('maintains data consistency across multiple API endpoints', async () => {
      ...
      const floorResponse = await queryFloorData(professionalApiKey, timeseriesData.floor_number)
      ...
      const meanDifference = Math.abs(summaryData.statistics.mean - calculatedMean)
  ...
  describe('Data Export Integration Workflows', () => {
    test('complete data export lifecycle with all formats', async () => {
      ...
      for (const job of exportJobs) {
        ...
        while (!completed && attempts < maxAttempts) {
          const statusResponse = await checkExportProgress(professionalApiKey, job.jobId)
          ...
          if (statusResponse.body.status === 'completed') {
            ...
            // Test download
            const downloadResponse = await downloadExport(professionalApiKey, job.jobId)
            expect(downloadResponse.status).toBe(200)
            expect(downloadResponse.body.download_url).toMatch(/^https:\/\//)
            expect(downloadResponse.body.expires_at).toBeDefined()

          } else if (statusResponse.body.status === 'failed') {
            throw new Error(`Export job ${job.jobId} failed: ${statusResponse.body.error}`)
          }
    ...
    test('handles large dataset exports with proper chunking', async () => {
      ...
      const largeExportResponse = await createDataExport(professionalApiKey, {
        format: 'csv',
        data_type: 'timeseries',
        sensors: Array.from({ length: 20 }, (_, i) => `SENSOR_${String(i + 1).padStart(3, '0')}`),
        date_range: {
      ...
      for (let i = 0; i < 20; i++) {
        const statusResponse = await checkExportProgress(professionalApiKey, jobId)
        expect(statusResponse.status).toBe(200)
  ...
  describe('Webhook Integration Workflows', () => {
    test('complete webhook lifecycle with event filtering', async () => {
      ...
      const dataUpdateWebhook = receivedWebhooks.find(w => w.event_type === 'data.updated')
      const alertWebhook = receivedWebhooks.find(w => w.event_type === 'alert.triggered')
    ...
    test('webhook retry mechanism with failure recovery', async () => {
      ...
      const analyticsResponse = await getWebhookAnalytics(professionalApiKey, webhook.id)
      expect(analyticsResponse.status).toBe(200)
      expect(analyticsResponse.body.delivery_stats.total_deliveries).toBe(1)
      expect(analyticsResponse.body.delivery_stats.successful_deliveries).toBe(1)
    })
  })
  ...
  describe('Cross-System Integration Validation', () => {
    ...
    test('NextAuth authentication integration', async () => {
      ...
      const apiKeyCreationResponse = await createApiKeyThroughSession(sessionToken, {
      ...
      const expiredSessionResponse = await createApiKeyThroughSession(sessionToken, {
  ...
  describe('Performance Integration Tests', () => {
    ...
    test('system stability under sustained load', async () => {
      ...
      console.log(`Completed ${results.length} requests over ${loadDurationMs / 1000}s`)
