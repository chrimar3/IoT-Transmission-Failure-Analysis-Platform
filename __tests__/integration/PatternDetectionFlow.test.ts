...
const mockGetSensorData = BangkokDataService.getSensorData as jest.MockedFunction<typeof BangkokDataService.getSensorData>
const mockGetEquipmentInfo = BangkokDataService.getEquipmentInfo as jest.MockedFunction<typeof BangkokDataService.getEquipmentInfo>
...
describe('Pattern Detection Integration Flow', () => {
  ...
  const mockSensorData = [
    { timestamp: '2025-01-15T08:00:00Z', value: 22.5, sensor_id: 'SENSOR_HVAC_001' },
    { timestamp: '2025-01-15T08:15:00Z', value: 22.8, sensor_id: 'SENSOR_HVAC_001' },
    { timestamp: '2025-01-15T08:30:00Z', value: 45.2, sensor_id: 'SENSOR_HVAC_001' }, // Anomaly
    { timestamp: '2025-01-15T08:45:00Z', value: 23.1, sensor_id: 'SENSOR_HVAC_001' },
    { timestamp: '2025-01-15T09:00:00Z', value: 22.9, sensor_id: 'SENSOR_HVAC_001' }
  ]
  ...
  describe('End-to-End Pattern Detection and Acknowledgment Flow', () => {
    it('should complete full pattern detection and acknowledgment workflow', async () => {
      ...
      const detectRequest = new NextRequest('http://localhost/api/patterns/detect', {
        method: 'POST',
        body: JSON.stringify({
      ...
      const acknowledgeRequest = new NextRequest('http://localhost/api/patterns/acknowledge', {
        method: 'POST',
        body: JSON.stringify({
          pattern_id: detectedPattern.id,
          notes: 'Investigated temperature spike, appears to be sensor calibration issue',
      ...
      expect(acknowledgeData.data.acknowledgment.pattern_id).toBe(detectedPattern.id)
      expect(acknowledgeData.data.acknowledgment.acknowledged_by).toBe(mockUser.email)
      expect(acknowledgeData.data.acknowledgment.notes).toBe('Investigated temperature spike, appears to be sensor calibration issue')
    ...
    it('should handle multiple sensors with different patterns', async () => {
      ...
      mockGetSensorData.mockResolvedValueOnce([
        ...
        { timestamp: '2025-01-15T08:00:00Z', value: 22.5, sensor_id: 'SENSOR_HVAC_001' },
        { timestamp: '2025-01-15T08:15:00Z', value: 45.0, sensor_id: 'SENSOR_HVAC_001' }, // Spike
        { timestamp: '2025-01-15T08:30:00Z', value: 22.8, sensor_id: 'SENSOR_HVAC_001' },
        // Lighting sensor with gradual increase trend
        { timestamp: '2025-01-15T08:00:00Z', value: 150, sensor_id: 'SENSOR_LIGHT_001' },
        { timestamp: '2025-01-15T08:15:00Z', value: 155, sensor_id: 'SENSOR_LIGHT_001' },
        { timestamp: '2025-01-15T08:30:00Z', value: 160, sensor_id: 'SENSOR_LIGHT_001' },
        { timestamp: '2025-01-15T08:45:00Z', value: 165, sensor_id: 'SENSOR_LIGHT_001' } // Trend
      ])
    ...
    it('should prevent duplicate pattern acknowledgments', async () => {
      ...
      if (patternId) {
        ...
        const acknowledgeRequest1 = new NextRequest('http://localhost/api/patterns/acknowledge', {
          method: 'POST',
          body: JSON.stringify({
        ...
        const acknowledgeRequest2 = new NextRequest('http://localhost/api/patterns/acknowledge', {
          method: 'POST',
          body: JSON.stringify({
            pattern_id: patternId,
    ...
    it('should provide comprehensive error information for debugging', async () => {
      ...
      mockGetSensorData.mockRejectedValue(new Error('Database connection failed'))
    ...
    it('should handle malformed request data gracefully', async () => {
      const invalidRequest = new NextRequest('http://localhost/api/patterns/detect', {
        method: 'POST',
        body: 'invalid json{'
      })
    ...
    it('should validate follow-up requirements in acknowledgments', async () => {
      ...
      expect(data.message).toBe('Follow-up date must be specified when follow-up is required')
    })
  })
  ...
  describe('Performance and Reliability', () => {
    ...
    it('should handle data service timeouts gracefully', async () => {
      ...
      mockGetSensorData.mockImplementation(() =>
        new Promise((resolve) => setTimeout(() => resolve(mockSensorData), 100))
      )
  ...
  describe('Business Logic Validation', () => {
    it('should generate appropriate recommendations based on equipment type', async () => {
      ...
      if (data.data.patterns.length > 0) {
        ...
        if (pattern.recommendations.length > 0) {
          ...
          const recommendationText = pattern.recommendations[0].description.toLowerCase()
          expect(
    ...
    it('should respect confidence thresholds', async () => {
      const highThresholdRequest = new NextRequest('http://localhost/api/patterns/detect', {
        method: 'POST',
        body: JSON.stringify({
          sensor_ids: ['SENSOR_HVAC_001'],
          time_window: '24h',
