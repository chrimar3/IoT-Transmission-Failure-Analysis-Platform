...
jest.mock('@/components/ui/card', () => ({
  Card: ({ children, className, onClick }: unknown) => (
  ...
  CardDescription: ({ children }: unknown) => <div data-testid="card-description">{children}</div>,
...
jest.mock('@/components/ui/button', () => ({
  Button: ({ children, onClick, variant, size, disabled }: unknown) => (
    <button
      onClick={onClick}
      disabled={disabled}
...
jest.mock('@/components/ui/dialog', () => ({
  Dialog: ({ children, open, onOpenChange }: unknown) => (
    open ? <div data-testid="dialog" onBlur={() => onOpenChange?.(false)}>{children}</div> : null
  ),
  DialogContent: ({ children }: unknown) => <div data-testid="dialog-content">{children}</div>,
  DialogDescription: ({ children }: unknown) => <div data-testid="dialog-description">{children}</div>,
  DialogFooter: ({ children }: unknown) => <div data-testid="dialog-footer">{children}</div>,
  DialogHeader: ({ children }: unknown) => <div data-testid="dialog-header">{children}</div>,
  DialogTitle: ({ children }: unknown) => <div data-testid="dialog-title">{children}</div>
}))

jest.mock('@/components/ui/textarea', () => ({
  Textarea: ({ value, onChange, placeholder, ...props }: unknown) => (
    <textarea
      value={value}
      onChange={onChange}
...
jest.mock('@/components/ui/input', () => ({
  Input: ({ value, onChange, placeholder, ...props }: unknown) => (
    <input
      value={value}
      onChange={onChange}
      placeholder={placeholder}
...
jest.mock('@/components/ui/label', () => ({
  Label: ({ children, htmlFor }: unknown) => (
...
jest.mock('@/components/ui/scroll-area', () => ({
  ScrollArea: ({ children }: unknown) => <div data-testid="scroll-area">{children}</div>
}))
...
describe('PatternDetectionWidget', () => {
      ...
      recommendations: [
        {
          ...
          description: 'Inspect HVAC temperature sensors for calibration issues',
          estimated_cost: 500,
  ...
  describe('rendering', () => {
    it('should render patterns correctly', () => {
      ...
      expect(screen.getByText('HVAC Temperature Anomaly - Readings 15% above normal range')).toBeInTheDocument()
      expect(screen.getByText('Lighting Power Consumption Spike - 40% above normal')).toBeInTheDocument()
    ...
    it('should show empty state when no patterns', () => {
      ...
      expect(screen.getByText('All systems operating within normal parameters')).toBeInTheDocument()
    })
  ...
  describe('interactions', () => {
    it('should call onPatternSelect when pattern is clicked', async () => {
      ...
      expect(defaultProps.onPatternSelect).toHaveBeenCalledWith(mockPatterns[0])
    })
    ...
    it('should handle modal form submission', async () => {
      const user = userEvent.setup()
      defaultProps.onAcknowledge.mockResolvedValue(undefined)
      ...
      // Fill form
      const notesTextarea = screen.getByPlaceholderText('Add any observations or additional context...')
      const actionInput = screen.getByPlaceholderText('e.g., Schedule maintenance inspection')
      ...
      await waitFor(() => {
        expect(defaultProps.onAcknowledge).toHaveBeenCalledWith(
          'pattern_001',
    ...
    it('should handle acknowledge loading state', async () => {
      const user = userEvent.setup()
      const slowAcknowledge = jest.fn(() => new Promise(resolve => setTimeout(resolve, 100)))

      render(<PatternDetectionWidget {...defaultProps} onAcknowledge={slowAcknowledge} />)
  ...
  describe('edge cases', () => {
    it('should handle patterns without data points', () => {
      ...
      render(<PatternDetectionWidget {...defaultProps} patterns={patternsWithoutData} />)
    ...
    it('should handle patterns without recommendations', () => {
      ...
      render(<PatternDetectionWidget {...defaultProps} patterns={patternsWithoutRecs} />)
    ...
    it('should handle acknowledge errors gracefully', async () => {
      ...
      defaultProps.onAcknowledge.mockRejectedValue(new Error('Network error'))
    ...
    it('should handle very long descriptions', () => {
      ...
      render(<PatternDetectionWidget {...defaultProps} patterns={[longDescriptionPattern]} />)
    ...
    it('should handle missing equipment type icons', () => {
      ...
      render(<PatternDetectionWidget {...defaultProps} patterns={[unknownEquipmentPattern]} />)
  ...
  describe('performance', () => {
    it('should handle large numbers of patterns', () => {
      ...
      render(<PatternDetectionWidget {...defaultProps} patterns={manyPatterns} />)
    ...
    it('should not cause memory leaks with frequent updates', () => {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        rerender(<PatternDetectionWidget {...defaultProps} patterns={updatedPatterns} />)
      }
